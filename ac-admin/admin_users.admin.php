...
if(isset($_POST["delete_it"])){
	...
	$sql="SELECT id FROM ".T_BOOKINGS_ITEMS." WHERE id_user=".mysqli_real_escape_string($db_cal,$_POST["id"])."";
	$res=mysqli_query($db_cal,$sql);
	while($row=mysqli_fetch_assoc($res)){
		echo "<br>".$del="DELETE FROM ".T_BOOKINGS." WHERE id_item=".mysqli_real_escape_string($db_cal,$row["id"])." LIMIT 1";
		mysqli_query($db_cal,$del) or die("Error deelting item bookings");
	}
	//	delete calendar items
	$del="DELETE FROM ".T_BOOKINGS_ITEMS." WHERE id_user=".mysqli_real_escape_string($db_cal,$_POST["id"])."";
...
if(isset($_POST["add"])){
	...
	if(add_item($this_table,$_POST["add"],false)) 			header("Location:index.php?page=".ADMIN_PAGE."&id_added=".mysqli_insert_id($db_cal)."&msg=add_OK");
