...
$error=0;
	...
	*/
	...
	$res_db = mysqli_query($db_cal,$sql_db) or die("Error getting DB TABLES<br>".mysqli_error($db_cal));
	while ($row_db = mysqli_fetch_row($res_db)) {
		//echo "Table: {$row[0]}\n";
	   	// now get fields within tables
	   	$this_table=$row_db[0];
		$sql_tables="SHOW COLUMNS FROM ".$row_db[0]."";
	  	$res_table = mysqli_query($db_cal,$sql_tables) or die("Error getting TABLE COLUMNS");
		if (mysqli_num_rows($res_table) > 0) {
			...
			while ($row_table= mysqli_fetch_assoc($res_table)) {
				...
				if(substr($this_field,-3,3)=="_en"){
					...
					if($res_check=mysqli_query($db_cal,$sql_check)){
						...
						mysqli_query($db_cal,$add) or die("Error ADDING AC_LANGUAGE");
