...
if(isset($_POST["delete_it"])){
	if(delete_item($this_table,$_POST["id"])){
		...
		$del="DELETE FROM ".T_BOOKINGS." WHERE id_item=".mysqli_real_escape_string($db_cal,$_POST["id"])."";
...
if(!isset($_REQUEST["action"])){
	...
	if(mysqli_num_rows($res)==0)	$start_message=$lang["warning_no_calendar_items"];
