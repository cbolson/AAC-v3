...
function draw_cal($id_item,$month,$year,$manage_type="avail"){
	...
	";
	//echo $sql;
	if(!$res=mysqli_query($db_cal,$sql))	die("ERROR checking id item availability dates<br>".mysqli_error($db_cal));
...
function get_cal_update_date($id_item){
	...
	$sql="SELECT DATE_FORMAT(date_mod, '".$date_format."') as date_mod FROM `".T_BOOKING_UPDATE."` WHERE id_item=".mysqli_real_escape_string($db_cal,$id_item)."";
...
function sel_list_items($id_item_current){
	...
	$res=mysqli_query($db_cal,$sql) or die("Error checking items");
	while($row=mysqli_fetch_assoc($res)){
...
function itemTitle($id){
	global $db_cal;
	$sql="SELECT desc_".AC_LANG." as item_title FROM ".T_BOOKINGS_ITEMS." WHERE id=".mysqli_real_escape_string($db_cal,$id)."";
