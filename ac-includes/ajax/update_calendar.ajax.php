...
$res=mysqli_query($db_cal,$sql) or die("Error getting states");
...
if($_GET["id_state"]=="free"){
	...
	$res=mysqli_query($db_cal,$sql) or die("Error getting date data");
	...
	if(mysqli_num_rows($res)==0){
		...
		$update		=	"INSERT INTO ".T_BOOKINGS." SET id_item='".mysqli_real_escape_string($db_cal,$id_item)."',the_date='".mysqli_real_escape_string($db_cal,$the_date)."', id_state='".mysqli_real_escape_string($db_cal,$new_state)."'";
	}else{
		...
		//echo "<br>Current ID: ".$current_state_id;
		//print_r($list_states);
		...
		//echo "<br>New State: ".$new_state;
		if($new_state==""){
			//	finished array - delete from db
			$update="DELETE FROM ".T_BOOKINGS." WHERE id_item='".mysqli_real_escape_string($db_cal,$id_item)."' AND the_date='".mysqli_real_escape_string($db_cal,$the_date)."' LIMIT 1";
			$new_class="";
			$new_desc	=$lang["available"];
		}else{
			$update="UPDATE ".T_BOOKINGS." SET id_state='".mysqli_real_escape_string($db_cal,$new_state)."' WHERE id_item='".mysqli_real_escape_string($db_cal,$id_item)."' AND the_date='".mysqli_real_escape_string($db_cal,$the_date)."' LIMIT 1";
...
if(!mysqli_query($db_cal,$update)) die("Error updating<br>".mysqli_error($db_cal)."<br>".$update);

//	update last update with now
$sql="SELECT * FROM `".T_BOOKING_UPDATE."` WHERE id_item='".mysqli_real_escape_string($db_cal,$id_item)."'";
if(!$res=mysqli_query($db_cal,$sql)) die("ERROR GETTING CHECKING UPDATE DATE.<br>".mysqli_error($db_cal));
if(mysqli_num_rows($res)==0)	$update="INSERT INTO `".T_BOOKING_UPDATE."` SET id_item='".mysqli_real_escape_string($db_cal,$id_item)."', date_mod=now()";
else							$update="UPDATE `".T_BOOKING_UPDATE."` SET date_mod=now() WHERE id_item='".mysqli_real_escape_string($db_cal,$id_item)."' LIMIT 1";
//echo $update;
mysqli_query($db_cal,$update) or die("Error with last modified date");
